...
-(void)rotate90CounterClockwise
{
    CGRect drawRect = CGRectMake(0, 0, changedPhoto.size.width,changedPhoto.size.height);
    UIGraphicsBeginImageContext(CGSizeMake(changedPhoto.size.height, changedPhoto.size.width));
    CGContextRef context = UIGraphicsGetCurrentContext();
    CGContextRotateCTM(context, radians(-90.0));
    CGContextTranslateCTM(context, -changedPhoto.size.width, 0);
    [changedPhoto drawInRect:drawRect];
    UIImage *image = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    changedPhoto = image;
    photoView.image=changedPhoto;
...
//        backupPhoto = changedPhoto;
//    }
//    changedPhoto = [UIImage imageWithCGImage: CGImageCreateCopy(backupPhoto.CGImage)];
//    photoView.image = changedPhoto;
...
//    adjustTabItem.tag = ADJUST_TABITEM_TAG;
    ...
    tabBarView.itemPositioning = UITabBarItemPositioningFill;
    tabBarView.items = [NSArray arrayWithObjects:clipTabItem,filterTabItem, nil];
...
-(void)loadTabUpViews
{
    ...
    CGFloat clipButtonDimension = 45;
    ...
    {
        UIButton *button = [[UIButton alloc]initWithFrame:CGRectMake(10+index*70, 20, 60, 50)];
    ...
    filterView.contentSize = CGSizeMake(10+index*70, viewHeight);
...
-(void)viewDidAppear:(BOOL)animated
{
    ...
    [self clipRatioWidth:originalPhoto.size.width Height:originalPhoto.size.height];
    isClippedLastTime = YES;
